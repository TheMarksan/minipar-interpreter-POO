...
SEQ {
  ...
  // Função auxiliar para processar resultado da REST API
  function processInterpretResult(data) {
    ...
    if(data.waiting_for_input){
      ...
      // Mostrar saída parcial primeiro
      const execText = data.saida || data.exec || data.execucao || data.stdout || data.output || '';
      execOut.textContent = (typeof execText === 'string' ? execText : JSON.stringify(execText,null,2)) || '';
      
      // Adicionar campo de input
      showInputPrompt(currentRunId, data.prompt || '');
    } else {
      // Não está esperando input, mostrar saída final
      removeInputPrompt();
      currentRunId = null;
  ...
  async function interpretarCodigo(){
    ...
    // Usar REST API
    try{
      // Auto-detectar protocolo, host e porta
      const protocol = window.location.protocol;
      const apiHost = window.location.hostname || 'localhost';
      const apiUrl = `${protocol}//${apiHost}:8000/interpretar`;
      const resp = await fetch(apiUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code})});
  ...
  async function sendRunInput(runId, value){
    if(!runId) return;
    try{
      // Auto-detectar protocolo e host
      const protocol = window.location.protocol; // http: ou https:
      const apiHost = window.location.hostname || 'localhost';
      const apiUrl = `${protocol}//${apiHost}:8000/interpretar/input`;
      const resp = await fetch(apiUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({run_id: runId, input: value})});
      const data = await resp.json();
